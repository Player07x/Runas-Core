...
Desde a versão 22, `CharacterInventoryItem` tem `size` (comprimento em centímetros) e `mt`, sempre derivado por `calculateItemSizeModifier` (tabela de tamanho da ficha + 2; uma espada de 80 cm tem MT +0). A normalização recalcula `mt` a partir de `size`; fichas anteriores migram com `size = 0` e `mt = 0`. Ao causar dano por um item, Tools e DM ativam `Aplicar MT` com o MT do item.
