...
- `/api/campaign-data` guarda o snapshot privado e normalizado de Campanhas e Wiki no D1; o estado primário continua no IndexedDB. O `PUT` grava exatamente o que recebe, sem mesclar — por isso o cliente sempre busca o snapshot atual e mescla com `mergeKnowledgeWorkspaces` (que respeita `deletedIds`) antes de gravar; do contrário, uma aba com estado desatualizado (por exemplo, que não viu uma exclusão feita alhures) reviveria o registro ao salvar por cima.
