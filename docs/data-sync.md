...
Campanhas e Wiki abrem sem login nem senha: os dados são locais (IndexedDB e, opcionalmente, o vault do Obsidian). O token de backup, o mesmo do Bestiário, serve apenas para ativar a cópia na nuvem. `/api/campaign-data` exige o mesmo `Authorization: Bearer <token>` de `/api/backup`.
...
O backup de Campanhas e Wiki (`/api/campaign-data`) é só de saída: o Runas DM nunca busca nem mescla o remoto sozinho. A cada mudança (criar, editar ou remover), o estado é salvo localmente e, pouco depois — somente se houver token de backup na sessão —, enviado por inteiro ao D1, sem ler o snapshot atual antes de gravar. O servidor grava exatamente o que recebe. Sem token, o backup fica desativado e nada sai do dispositivo; `Backup na nuvem` no menu `⋯` pede o token e ativa o envio.
