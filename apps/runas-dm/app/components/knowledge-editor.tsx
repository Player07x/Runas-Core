    ...
    <section className={`knowledge-editor ${isEncounter ? "encounter-editor" : ""}`} role="dialog" aria-modal="true" aria-labelledby="knowledge-editor-title">
      ...
      <div className="knowledge-editor-body">
        ...
        <aside>
          ...
          {draft.scope === "campaign" && <section><header><BookOpen size={16} /><strong>Configurações visuais</strong></header><div className="visual-settings"><label><span>Cor de destaque</span><input type="color" value={draft.accentColor || "#58a667"} onChange={(event) => patch({ accentColor: event.target.value })} /></label><label><span>Cor do fundo</span><input type="color" value={draft.backgroundColor || "#171316"} onChange={(event) => patch({ backgroundColor: event.target.value })} /></label><label><span>Cor do texto</span><input type="color" value={draft.textColor || "#f5eeee"} onChange={(event) => patch({ textColor: event.target.value })} /></label><label><span>Imagem de fundo</span><input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => patch({ backgroundImageDataUrl: String(reader.result) }); reader.readAsDataURL(file) }} /></label></div></section>}
