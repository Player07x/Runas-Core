...
export function KnowledgePortal({ area }: { area: PortalArea }) {
  ...
  const [gridDensity, setGridDensity] = useState<GridDensity>(() => {
    ...
    const saved = window.localStorage.getItem("runas-dm.grid-density")
  ...
  function changeGridDensity(value: GridDensity) {
    ...
    window.localStorage.setItem("runas-dm.grid-density", value)
  }
  return <main className={`knowledge-shell knowledge-app grid-size-${gridDensity} ${area === "campaigns" ? "campaign-themed" : ""}`} style={area === "campaigns" ? campaignTheme(selectedCampaign) : undefined}>
    ...
    {area === "campaigns" ? <CampaignPortal campaigns={state.campaigns} selectedCampaignId={selectedCampaignId} pageCounts={campaignPageCounts} onCreate={addCampaign} onReorder={reorderCampaigns} onSelect={(id) => { setSelectedCampaignId(id); navigateRoute({ campaignId: id, page: String(selectedKind) }) }}><>
      ...
      {selectedCampaign && <GridSizeControl value={gridDensity} onChange={changeGridDensity} />}
...
function GridSizeControl({ value, onChange }: { value: GridDensity; onChange: (value: GridDensity) => void }) {
  return <div className="grid-size-control" aria-label="Tamanho da grade"><span>Grade</span><button type="button" title="Diminuir tamanho da grade" aria-label="Diminuir tamanho da grade" disabled={value === "small"} onClick={() => onChange("small")}><Minus size={13} /></button><button type="button" title="Tamanho padrão da grade" aria-label="Tamanho padrão da grade" aria-pressed={value === "medium"} onClick={() => onChange("medium")}><Grid2X2 size={13} /></button><button type="button" title="Aumentar tamanho da grade" aria-label="Aumentar tamanho da grade" disabled={value === "large"} onClick={() => onChange("large")}><Plus size={13} /></button></div>
}
