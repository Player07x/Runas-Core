...
import { conflictCopies, removePagesById } from "../lib/conflict-copies"
...
export function KnowledgePortal({ area }: { area: PortalArea }) {
  ...
  const conflictPages = useMemo(() => conflictCopies(state.pages), [state.pages])
  ...
  /** Remove cópias em conflito do site e, com a integração ativa, os arquivos delas no vault (a versão fica em Assets/Runas DM Backups). */
  function removeConflicts(targets: KnowledgePage[], scope: string) {
    if (targets.length === 0) return
    const withFile = targets.filter((page) => page.obsidianPath)
    const vaultNote = obsidianPreferences.enabled && withFile.length > 0 ? ` Os ${withFile.length} arquivo${withFile.length === 1 ? "" : "s"} correspondente${withFile.length === 1 ? "" : "s"} no vault também vão embora; uma cópia de cada fica em Assets/Runas DM Backups.` : ""
    if (!window.confirm(`Remover ${targets.length} cópia${targets.length === 1 ? "" : "s"} em conflito ${scope}?${vaultNote}`)) return
...
function ConflictCopiesNotice({ count, onRemoveAll }: { count: number; onRemoveAll: () => void }) {
  return <aside className="knowledge-warning outside-pages-notice"><header><div><strong>Cópias em conflito ({count})</strong><p>O site guarda uma cópia quando a mesma página muda no site e no vault ao mesmo tempo. Remova todas de uma vez; cada arquivo do vault é guardado em Assets/Runas DM Backups antes de sair.</p></div><button className="outside-remove-all" onClick={onRemoveAll}>Remover todas as cópias</button></header></aside>
}
