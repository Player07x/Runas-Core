...
export function KnowledgePortal({ area }: { area: PortalArea }) {
  ...
  return <main className={`knowledge-shell knowledge-app grid-size-${gridDensity} ${area === "campaigns" ? "campaign-themed" : ""}`} style={area === "campaigns" ? campaignTheme(selectedCampaign) : undefined}>
    <KnowledgeHeader area={area} syncState={syncState} gridDensity={gridDensity} onChangeGridDensity={changeGridDensity} onObsidian={() => setObsidianOpen(true)} onCloudBackup={() => requestCloudAction("backup")} onCloudImport={() => requestCloudAction("import")} />
...
function KnowledgeHeader({ area, syncState, gridDensity, onChangeGridDensity, onObsidian, onCloudBackup, onCloudImport }: { area: PortalArea; syncState: SyncState; gridDensity: GridDensity; onChangeGridDensity: (value: GridDensity) => void; onObsidian: () => void; onCloudBackup: () => void; onCloudImport: () => void }) {
