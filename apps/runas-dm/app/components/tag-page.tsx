"use client"
import { conflictCopies } from "../lib/conflict-copies"
...
export function TagPage({ pages, section, tag, onOpen, onCreate, onBack, onRemoveConflicts }: { pages: KnowledgePage[]; section: string; tag: string; onOpen: (page: KnowledgePage) => void; onCreate?: () => void; onBack?: () => void; onRemoveConflicts?: (conflicts: KnowledgePage[], scope: string) => void }) {
  const [query, setQuery] = useState(""); const [sort, setSort] = useState("recent")
  const isStory = section === "story" && !tag
  const tagConflicts = useMemo(() => isStory ? [] : conflictCopies(pagesForTag(pages, section, tag)), [pages, section, tag, isStory])
  /**
