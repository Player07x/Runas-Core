...
export function CampaignAdventure({ section, onSection, onBack, children, organizer, onOrganizerChange, counts = {} }: { section?: CampaignAdventureSection; onSection: (section: CampaignAdventureSection) => void; onBack: () => void; children?: ReactNode; organizer?: { nodes: OrganizerNode[]; edges: OrganizerEdge[] }; onOrganizerChange?: (nodes: OrganizerNode[], edges: OrganizerEdge[]) => void; counts?: Partial<Record<CampaignAdventureSection, number>> }) {
  if (!section) return <section className="campaign-subpage"><h2 className="section-title">Aventura</h2><div className="subpage-grid">{SECTIONS.map(({ id, label, description, icon: Icon }) => <button className="subpage-card" key={id} onClick={() => onSection(id)}><Icon size={24} /><strong>{label}</strong><span>{description}</span><small>{counts[id] ?? 0} {counts[id] === 1 ? "registro" : "registros"}</small></button>)}</div></section>
