  ...
  return (
    <div className="mt-3 flex w-full max-w-[12.5rem] flex-col gap-2 rounded-[18px] border border-border bg-muted/30 p-3">
      <div className="flex items-center gap-3">
        <span className="grid size-14 shrink-0 place-items-center overflow-hidden rounded-full border border-border bg-[repeating-conic-gradient(#0000000f_0_25%,transparent_0_50%)] bg-[length:12px_12px]">
...
function TokenDialog({ file, initialSize, onCancel, onConfirm }: { file: File; initialSize: number; onCancel: () => void; onConfirm: (image: string, size: number) => void }) {
  ...
  const [ringColor, setRingColor] = useState("#343b4e")
  ...
  const maxX = ((width - crop.size) / 2) * scale
  const maxY = ((height - crop.size) / 2) * scale
    ...
    <section role="dialog" aria-modal="true" aria-label="Editar token do personagem" onMouseDown={(event) => event.stopPropagation()} className="mx-auto my-2 w-full max-w-3xl rounded-[26px] border border-border bg-card p-4 shadow-2xl sm:my-4 sm:p-6">
      ...
      <div className="grid gap-6 sm:grid-cols-[300px_minmax(0,1fr)]">
        <div className="relative size-[300px] cursor-grab touch-none select-none overflow-hidden rounded-2xl bg-[#090b0c] active:cursor-grabbing"
          ...
          <span aria-hidden="true" className={`pointer-events-none absolute inset-0 border border-white/50 ${shape === "circle" ? "rounded-full shadow-[0_0_0_999px_rgb(8_10_18/0.62)]" : "rounded-2xl"}`} />
        </div>
        <div className="grid content-start gap-4 text-sm">
          ...
          {shape === "circle" && <div className="flex items-center justify-between gap-3"><label className="flex items-center gap-2 text-muted-foreground"><input type="checkbox" checked={ringEnabled} onChange={(event) => setRingEnabled(event.target.checked)} /> Borda</label><input type="color" aria-label="Cor da borda" value={ringColor} disabled={!ringEnabled} onChange={(event) => setRingColor(event.target.value)} className="h-8 w-12 cursor-pointer rounded-lg border border-border bg-transparent p-0" /></div>}
          ...
          <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onCancel}>Cancelar</Button><Button type="button" disabled={!image} onClick={() => { if (image) onConfirm(renderTokenImage({ image, crop, shape, ringColor: ring }), size) }}>Aplicar token</Button></div>
        </div>
      </div>
    </section>
  </div>
